{
  "eXtended Pooled List" (TObject>nil)
...
{ eXtended Pooled List (TObject>nil) }
  tXObjList=class(TRtcFastObject)
  private
    myPoolSize:longint;
...
procedure tXObjList.extractFirst(var exInfo:infoType);
  var
    x:pnode;
  begin
  if fFirst<>nil then
...
procedure tXObjList.extractLast(var exInfo:infoType);
  var
    x:pnode;
  begin
  if fLast<>nil then
