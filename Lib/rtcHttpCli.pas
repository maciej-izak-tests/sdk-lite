...
procedure TRtcHttpClient.SetCliRequest(const Value: TRtcClientRequest);
  ...
  {$IFDEF RTC_useWinHttp}
    if Con is TWinHttpProv then
      ...
      raise ERtcConnection.Create('TRtcHttpClient.SetRequest: Unsupported connection provider.');
  end;
...
procedure TRtcHttpClient.SetCliResponse(const Value: TRtcClientResponse);
  ...
  {$IFDEF RTC_useWinHttp}
    if Con is TWinHttpProv then
      ...
      raise ERtcConnection.Create('TRtcHttpClient.SetResponse: Unsupported connection provider.');
  end;
