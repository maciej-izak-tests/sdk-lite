...
{ eXtended Pooled List (RtcIntPtr>0) }
  ...
  public
    ...
    procedure extractFirst(var exInfo:infoType);
    procedure extractLast(var exInfo:infoType);
...
function tXList.Empty:boolean;
  begin
  Result:= (cnt=0);
  end;

function tXList.New_Node(const i:infoType; const pri,nex:pnode):pnode;
  var
    a:longint;
    p:pnodearr;
  begin
...
procedure tXList.PoolSize(size:integer);
// PoolSize;
  begin
...
procedure tXList.Del_Node(node:pnode);
// del_node
  begin
  if myPoolSize>0 then
    pool.Put(node)
...
constructor tXList.Create(size:integer);
// Create
  begin
  inherited Create;
  cnt:=0;
...
function tXList.Count: cardinal;
  begin
  Result:=cnt;
  end;

procedure tXList.addFirst(const info: infoType);
  var
    nn:pnode;
  begin
...
procedure tXList.addLast(const info: infoType);
  var
    nn:pnode;
  begin
...
function tXList.First: infoType;
  begin
  if fFirst=nil then
    Result:=infoNil
  else
...
procedure tXList.extractFirst(var exInfo:infoType);
  var
    x:pnode;
  begin
  if fFirst<>nil then
...
procedure tXList.extractLast(var exInfo:infoType);
  var
    x:pnode;
  begin
  if fLast<>nil then
...
procedure tXList.removeThis(const info: infoType);
  var
    x:pnode;
  begin
  x:=fFirst;
