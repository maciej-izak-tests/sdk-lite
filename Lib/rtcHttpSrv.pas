...
procedure TRtcHttpServer.WriteEx(const s: RtcByteArray);
  ...
  if isWebSocket then
    begin
    if State=conActive then Con.WriteEx(s);
    end
...
procedure TRtcHttpServer.Write(const s: RtcString);
  ...
  if isWebSocket then
    begin
    if State=conActive then Con.Write(s);
    end
  else if State>=conActive then
    begin
    if Response.Sent then
...
procedure TRtcHttpServer.SetSrvRequest(const Value: TRtcServerRequest);
  ...
  {$IFDEF RTC_useSockets}
    if Con is TSocketProv then
      ...
      raise ERtcConnection.Create('TRtcHttpServer.SetRequest: Connection Provider not recognized.');
  end;
...
procedure TRtcHttpServer.SetSrvResponse(const Value: TRtcServerResponse);
  ...
  {$IFDEF RTC_useSockets}
    if Con is TSocketProv then
      ...
      raise ERtcConnection.Create('TRtcHttpServer.SetResponse: Connection Provider not recognized.');
  end;
